...
def test_aaa():
    ...
    linear = chainer.functions.connection.linear.linear

    @as_funcnode('DynamicBatchReshapeLinear')
    def linear2(x, W, b=None, n_batch_axes=1):
        return linear(x, W, b, n_batch_axes)

    chainer.functions.connection.linear.linear = linear2
